...
echo      - "80:80"
echo    volumes:
echo      - #ABSOLUTE_PATH/%PROJECT_NAME%/app:/var/www/html
...
echo    volumes:
echo      - #ABSOLUTE_PATH/%PROJECT_NAME%/app:/var/www/html
echo      - ./docker/php/php/ini:/user/local/etc/php/conf.d/custom.ini
echo.
echo.
echo  mysql:
...
echo    volumes:
echo      - #ABSOLUTE_PATH/%PROJECT_NAME%/database:/var/lib/mysql
echo      - ./dockermysql/init:/docker-entrypoint-initdb.d
) > %PROJECT_NAME%\docker-compose.yml

echo "<?php echo %PROJECT_NAME%" > %PROJECT_NAME%\app\public\index.php
pause